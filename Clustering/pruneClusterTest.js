/*
 * THIS IS A ONE PAGE APP, index.js CONTAINS SITEWIDE JAVASCRIPT
 *
*/
...
<head>
  ...
  <script src="pruneClusterTest.js"></script>
</head>
...
// the URL of the WMS for dynamic data
// see also the OVERLAYS declaration
// var WMS_URL = 'http://websites.greeninfo.org/cgi-bin/mapserv?map=/var/www/coal_swarm/coal_tracker/wms/wms.map';
...
var BASEMAPS = {
    'hybrid' : L.tileLayer('http://{s}.tiles.mapbox.com/v3/greeninfo.map-zudfckcw/{z}/{x}/{y}.jpg', { zIndex:1 }),
    'satellite' : new L.Google(),
    'basemap' : L.tileLayer('http://{s}.tiles.mapbox.com/v3/greeninfo.map-jn70s6ih/{z}/{x}/{y}.jpg', { zIndex:1 })
};

// non-basemap overlay layers
// var zindex = 9;
// var OVERLAYS = {
//     // coalplants: the coal plants from the tracker database
//     'coalplants' : L.tileLayer.wms( WMS_URL, { layers: 'coalplants', transparent: true, opacity: 0.7, format: 'image/png', zIndex:zindex++ }),
//     // 'selected_coalplants' : L.tileLayer.wms( WMS_URL, { layers: 'selected_coalplants', transparent: true, opacity: 0.7, format: 'image/png', country: 'India', zIndex:zindex++ }),
// }
...
var COLUMN_NAMES = {
    ...
    'Annual CO2 (Mt/year)': 'annual_co2_mtons'
};
...
var BALKANS = ['Croatia', 'Romania', 'Serbia', 'Albania', 'Bulgaria', 'Greece', 'Montenegro', 'Slovenia', 'Bosnia & Herzegovina', 'FYROM'];

// the Unerse of status types: these are the categories used to symbolize coal plants on the map
//          key: this matches the status name in the trackers table and the trackers.json
//          id: simple id, used by PruneCluster
...
var STATUS_TYPES = {
    ...
    'pre-permit development': {'id': 1, 'text': 'Pre-permit development', 'color': '#ffa500', 'colorClass': 'orange', 'visible':true},
...
var colors = $.map(STATUS_TYPES, function(val, i) { return val.color });
...
$.fn.hasHorizontalScrollBar = function() {
    return this.get(0).scrollWidth > this.get(0).clientWidth; // for horizontal scrll
}
...
function initMap() {
    ...
    $.get('application/views/site/countries_simpl_v1.json', {}, function (data) {
...
function initPruneCluster() {
    // create a new PruceCluster object, with a minimal cluster size of (2)
    // in general we want the smallest clusters possible to show only clusters where there are multiple units in a single plant
    // CLUSTERS = new PruneClusterForLeaflet(2); // initial value
    CLUSTERS = new PruneClusterForLeaflet(30); // updated arg to 30; seems to really help countries like China/India
    ...
    // we don't force zoom to a cluster on click (the default): this is annoying
...
function initNavBar() {
    ...
    $('a[data-toggle="tab"]').on('show.bs.tab', function (e) {
        ...
        // also hiding the search icon for now, until we come up with a search strategy for tables
        // TO DO: Extend so that any shown sidebar hides (assuming there be others)
        //$('#nav-panel-search-button').hide();
    ...
    $('.nav-menu-button').click(function() {
        ...
        // // more cleanup: basically, this serves to 'reset' the app, visually (in all but file downloads)
        // // switch to map view
        // may be too laggy in FF ?? The problem is that if we start the search from the table, the map gets messed up; doesn't zoom to correct bounds, etc.
        // resetTheMap();

        // now, open the startup modal
        // $('#startup-modal').modal();
...
function initFreeSearch() {
    ...
    // triggered in two places: input on the sidebar, and input on the navbar
...
function _geocoder(address) {
    ...
    var url    = 'http://dev.virtualearth.net/REST/v1/Locations/' + encodeURIComponent(address);
...
function performSearch(type,name,extra) {
    ...
    var data_trackers;
    var requests = [];
    requests.push($.get('application/views/site/trackers_v1.json', {}, function (data) {
        data_trackers = data;
    }, 'json'));
...
function searchRegion(name, data_countries, data_trackers) {
    ...
    for (var i=0, l=data_countries.features.length; i<l; i++) {
        ...
        switch (name) {
            case 'non-EU Europe':
                if (data_countries.features[i].properties.REGION != 'non-EU Europe' && ( $.inArray( data_countries.features[i].properties.NAME, BALKANS) == -1) ) continue;
                break;
            case 'Balkans':
                if ( $.inArray( data_countries.features[i].properties.NAME, BALKANS) == -1) continue;
                break;
            default:
